/*
 * Tests for getcolsb.c - signed byte (TSBYTE) read functions
 */
...
#define test_path "test_getcolsb.fits"
...
test_read_primary_with_null_flags(void)
{
	...
	call_03(ffopen, &f, test_path, READONLY);
	call_07(ffgpfsb, f, 1, 1, 5, result, nularray, &anynull);
	fail_if(result[0] != 10);
	fail_if(result[1] != 20);
	fail_if(result[2] != 30);
	fail_if(result[3] != 40);
...
test_read_2d_array(void)
{
	...
	call_06(ffp2dsb, f, 1, 3, 3, 2, data);
	call_01(ffclos, f);

	call_03(ffopen, &f, test_path, READONLY);
	call_08(ffg2dsb, f, 1, 0, 3, 3, 2, result, &anynull);
	fail_if(result[0] != 1);
	fail_if(result[1] != 2);
	fail_if(result[2] != 3);
	fail_if(result[3] != 4);
...
test_read_3d_array(void)
{
	...
	call_03(ffopen, &f, test_path, READONLY);
	call_10(ffg3dsb, f, 1, 0, 2, 2, 2, 2, 2, result, &anynull);
	fail_if(result[0] != 1);
	fail_if(result[7] != 8);
	call_01(ffclos, f);
}
...
test_read_subsection(void)
{
	...
	call_02(ffinit, &f, "!" test_path);
	call_04(ffphps, f, SBYTE_IMG, 2, naxes);
	call_05(ffpprsb, f, 1, 1, 16, data);
	call_01(ffclos, f);

	call_03(ffopen, &f, test_path, READONLY);
	call_10(ffgsvsb, f, 1, 2, naxes, fpixel, lpixel, inc, 0, result, &anynull);
	fail_if(result[0] != 6);
	fail_if(result[1] != 7);
	fail_if(result[2] != 10);
	fail_if(result[3] != 11);
...
test_read_binary_table_column(void)
{
	...
	call_08(ffgcvsb, f, 1, 1, 1, 3, 0, result, &anynull);
	fail_if(result[0] != -128);
	fail_if(result[1] != 0);
...
test_read_column_with_nulls(void)
{
	...
	call_03(ffopen, &f, test_path, READONLY);
	call_03(ffmahd, f, 2, NULL);
	call_08(ffgcvsb, f, 1, 1, 1, 5, 0, result, &anynull);
	fail_if(result[0] != 10);
	fail_if(result[1] != 20);
	fail_if(result[2] != 30);
	fail_if(result[3] != 40);
...
test_read_column_with_null_flags(void)
{
	...
	call_03(ffopen, &f, test_path, READONLY);
	call_03(ffmahd, f, 2, NULL);
	call_08(ffgcfsb, f, 1, 1, 1, 5, result, nularray, &anynull);
	fail_if(result[0] != 10);
	fail_if(result[1] != 20);
	fail_if(result[2] != 30);
	fail_if(result[3] != 40);
...
test_read_multirow(void)
{
	...
	char *ttype[] = { "SBCOL" };
	char *tform[] = { "3S" };
	signed char data[] = { 1, 2, 3, 4, 5, 6, 7, 8, 9 };
	...
	call_04(ffphps, f, BYTE_IMG, 0, NULL);
	call_08(ffcrtb, f, BINARY_TBL, 3, 1, ttype, tform, NULL, NULL);
	call_06(ffpclsb, f, 1, 1, 1, 9, data);
	call_01(ffclos, f);

	call_03(ffopen, &f, test_path, READONLY);
	call_03(ffmahd, f, 2, NULL);
	call_08(ffgcvsb, f, 1, 1, 1, 9, 0, result, &anynull);
	fail_if(result[0] != 1);
	fail_if(result[4] != 5);
	fail_if(result[8] != 9);
	call_01(ffclos, f);
...
test_read_variable_length_array(void)
{
	...
	char *ttype[] = { "VARSB" };
	char *tform[] = { "1PS" };
	signed char data1[] = { 10, 20, 30 };
	signed char data2[] = { 40, 50 };
	signed char result[5];
	int anynull;
	long nelem;
	...
	fail_if(nelem != 2);
	call_08(ffgcvsb, f, 1, 2, 1, 2, 0, &result[3], &anynull);
	fail_if(result[3] != 40);
	fail_if(result[4] != 50);
	call_01(ffclos, f);
}
...
test_short_overflow(void)
{
	...
	call_03(ffopen, &f, test_path, READONLY);
	call_03(ffmahd, f, 2, NULL);
	ffgcvsb(f, 1, 1, 1, 1, 0, result, &anynull, &status);
...
test_read_noncontiguous_3d(void)
{
	...
	call_03(ffopen, &f, test_path, READONLY);
	call_10(ffg3dsb, f, 1, 0, 3, 3, 2, 2, 2, result, &anynull);
	fail_if(result[0] != 1);
	fail_if(result[1] != 2);
	fail_if(result[3] != 3);
	fail_if(result[4] != 4);
...
test_bad_dimen_3d(void)
{
	...
	call_03(ffopen, &f, test_path, READONLY);
	ffg3dsb(f, 1, 0, 2, 2, 4, 4, 2, result, &anynull, &status);
...
test_read_group_parameters(void)
{
	...
	int status = 0;
	long naxes[] = { 4 };
	signed char pdata[] = { 10, 20 };
	signed char idata[] = { 1, 2, 3, 4 };
	signed char presult[2];
	int anynull;

	call_02(ffinit, &f, "!" test_path);
	call_08(ffphpr, f, TRUE, SBYTE_IMG, 1, naxes, 2, 1, TRUE);
	call_05(ffpgpsb, f, 1, 1, 2, pdata);
	call_05(ffpprsb, f, 1, 1, 4, idata);
	call_01(ffclos, f);

	call_03(ffopen, &f, test_path, READONLY);
	call_05(ffggpsb, f, 1, 1, 2, presult);
...
test_read_subsection_with_null_flags(void)
{
	...
	call_03(ffopen, &f, test_path, READONLY);
	call_10(ffgsfsb, f, 1, 2, naxes, fpixel, lpixel, inc, result, nularray, &anynull);
	fail_if(result[0] != 6);
	fail_if(result[1] != 7);
	fail_if(result[2] != 10);
	fail_if(result[3] != 11);
